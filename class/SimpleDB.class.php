...
class SimpleDB extends AbstractDatabase {
    ...
    private function sqlWhere($where, &$vars = array()) { //2011-06-25
        $sql = '';
        $vars = array();
        ...
        $validOperators = array  //valid operators and whether they support IN
            (
            '==' => true,
            '!=' => true,
            '>' => false,
        ...
        foreach ($where as $fieldName => $operatorObject) {
            ...
            if (!is_array($operatorObject))
                $operatorObject = array($operatorObject, '==');
    ...
    private function sqlOrderBy($orderBy) { //2011-06-25
        $sql = '';
    ...
    private function sqlLimit($limit) { //2011-06-25
        $sql = '';
    ...
    private function escape($field) { //2012-09-11
        return '`' . str_replace('`', '``', $field) . '`';
    }

    public function count($table, $where = null) { //2011-06-30
        return (int) $this->selectValue('COUNT(*)', $table, $where);
    }

    public function insert($table, $data = null) { //2011-06-25
        $fields = array();
        $values = array();
        ...
        $vars = (($data === null) ? array() : $data);
        $this->exec($sql, $vars);
        return $this->lastInsertID();
    }

    public function update($table, $data, $where = null) { //2011-06-25
    ...
    public function delete($table, $where = null) { //2011-06-25
        $sql = 'DELETE FROM ' . $this->escape($table) . $this->sqlWhere($where, $vars);

        return $this->exec($sql, $vars);
    }

    public function select($table, $where = null, $orderBy = null, $limit = null) { //2011-06-25
    ...
    public function selectValues($column, $table, $where = null, $orderBy = null, $limit = null) { //2013-05-03
    ...
    public function selectRow($table, $where = null, $orderBy = null) { //2011-06-25
        $sql = 'SELECT * FROM ' . $this->escape($table) . $this->sqlWhere($where, $vars) . $this->sqlOrderBy($orderBy);

        return $this->queryRow($sql, $vars);
    }

    public function selectValue($column, $table, $where = null, $orderBy = null) { //2011-06-25
    ...
    public function selectJoin($table1, $table2, $using, $where = null, $orderBy = null, $limit = null) { //2011-08-08
    ...
    public function selectJoinRow($table1, $table2, $using, $where = null, $orderBy = null) { //2011-11-02
    ...
    public function truncateTable($table) { //2012-09-11
        $sql = 'TRUNCATE TABLE ' . $this->escape($table);

        return $this->exec($sql);
    }

    public function min($column, $table, $where = null) { //2013-01-24
        $sql = 'SELECT MIN(' . $this->escape($column) . ') FROM ' . $this->escape($table) . $this->sqlWhere($where, $vars);

        return $this->queryValue($sql, $vars);
    }

    public function max($column, $table, $where = null) { //2013-01-24
        $sql = 'SELECT MAX(' . $this->escape($column) . ') FROM ' . $this->escape($table) . $this->sqlWhere($where, $vars);

        return $this->queryValue($sql, $vars);
    }

    public function sum($column, $table, $where = null) { //2013-01-24
    ...
    public function avg($column, $table, $where = null) { //2013-01-24
